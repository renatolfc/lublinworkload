...
struct Job_t {
    ...
    Job_t(){}
    Job_t(int id, double submit, double wait, double runtime, int size,
          double cpu, double mem, int reqsize, double estimate, double reqmem,
          short status, int uid, int gid, int executable, int queueid,
          int partition, int depjob, double think) : id(id), submit(submit),
    wait(wait), runtime(runtime), size(size), cpu(cpu), mem(mem),
    reqsize(reqsize), estimate(estimate), reqmem(reqmem), status(status),
    uid(uid), gid(gid), executable(executable), queueid(queueid),
    partition(partition), depjob(depjob), think(think)
    {}
